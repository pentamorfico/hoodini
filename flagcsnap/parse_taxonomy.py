    ...
    def run(self):        
        ...
        with console.status("[bold green]Making trees ...") as status:
            ...
            if self.tree_mode=="taxonomy":
                ...
                dendropy_tree = dendropy.Tree.get(data=tree, schema="newick")
                pdm = dendropy_tree.phylogenetic_distance_matrix().as_data_table()._data
                ...
                tree = dendropy.Tree.get(path=self.tree_file, schema="newick")
                pdm = tree.phylogenetic_distance_matrix().as_data_table()._data
                dispd = pd.DataFrame(index=range(len(targets)), columns=range(len(targets)))
                dispd.columns = dispd.index = targets
                for i in combinations_with_replacement(targets, 2):
                    dispd.loc[i[0], i[1]] = dispd.loc[i[1], i[0]]  = pdm[i[0].replace("_"," ")][i[1].replace("_"," ")]
                ...
                ps=subprocess.run(command, check=True,capture_output=True)
                tree = dendropy.Tree.get(data=ps.stdout.decode().strip(), schema="newick")
                pdm = tree.phylogenetic_distance_matrix().as_data_table()._data
                dispd = pd.DataFrame(index=range(len(targets)), columns=range(len(targets)))
                dispd.columns = dispd.index = targets
                for i in combinations_with_replacement(targets, 2):
