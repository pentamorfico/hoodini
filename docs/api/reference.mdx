...
| `--skip-emapper` | (flag) Skip downloading emapper/DIAMOND DB. |
...
| `--threads` | Number of threads for downloads and pigz (0 = use all cores). |
...
Download multiple assembly summary files concurrently and save combined table.
...
**Returns:**
    True if a new server was started, False if one was already alive.
...
- `send()`
  ...
  ```python
  def send(method: str, params: dict | None = None, timeout: float = DEFAULT_COMMAND_TIMEOUT, session_id: str | None = None) -> dict
  ```
...
- `wait_for_event()`
  ...
  ```python
  def wait_for_event(method: str, timeout: float = 30.0) -> bool
  ```
...
- `open_page()`
  ...
  ```python
  def open_page(timeout: float = 60.0) -> str
  ```
...
- `navigate()`
  ...
  ```python
  def navigate(session_id: str, url: str, timeout: float = 120.0) -> None
  ```
...
- `evaluate()`
  ...
  ```python
  def evaluate(session_id: str, expression: str, timeout: float = 30.0)
  ```
...
```python
def download_urls(urls, dest_dir, connections = 16, show_progress = True, out_names = None, num_threads: int = 0)
```
